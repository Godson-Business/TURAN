...
def test_executor_writes_a_local_remediation_note(workspace_temp_dir, monkeypatch) -> None:
    ...
    backup_path = workspace_temp_dir / "reports" / "backups" / "f1.conf.bak"
    assert execute_fix(finding, plan, policy, output_dir=workspace_temp_dir / "remediation", backup_path=backup_path) == "generated"
    artifact_path = workspace_temp_dir / "reports" / "generated" / "f1.conf"
    ...
    note_path = workspace_temp_dir / "remediation" / "f1.md"
    ...
    assert "reports/generated/f1.conf" in note_text
...
def test_executor_writes_category_specific_artifacts(workspace_temp_dir, monkeypatch) -> None:
    ...
    for index, (category, expected_snippet, expected_reason) in enumerate(scenarios, start=1):
        ...
        artifact_path = workspace_temp_dir / "reports" / "generated" / f"f{index}.conf"
        assert execute_fix(finding, plan, policy, output_dir=workspace_temp_dir / "remediation", backup_path=artifact_path.with_suffix(".conf.bak")) == "generated"
        artifact_text = artifact_path.read_text(encoding="utf-8")
