...
app = typer.Typer(
    ...
    help=(
        ...
        "- Compares two saved scan reports\n"
        ...
        "- `--interactive` lets you choose generate artifacts or fix locally, then pick fixes by number\n"
        ...
        "fix --local\n"
        "scan http://127.0.0.1:8000 --json-output reports\\scan.json\n"
        "scan http://127.0.0.1:8000 --markdown-output reports\\scan.md\n"
        "scan http://127.0.0.1:8000 --html-output reports\\scan.html\n"
        "report reports\\scan.json --html-output reports\\scan.html\n"
        "report reports\\scan.md\n"
        "report reports\\scan.html\n"
        "audit --audit-log reports\\audit.log\n"
        "audit --log-file reports\\audit.log\n"
        "audit --last 25\n"
        "audit --event scan\n"
        "audit --json-output reports\\audit.json\n"
        "doctor\n"
...
def scan(
    ...
    json_output_path = path_option_value(json_output)
    markdown_output_path = path_option_value(markdown_output)
    html_output_path = path_option_value(html_output)
    ...
    console.print(render_console(result, include_fix_plans=preview_fixes_enabled or interactive_enabled or generate_fixes_enabled))
    ...
    if interactive_enabled or generate_fixes_enabled:
        ...
        if interactive_enabled:
            console.print(render_interactive_fix_catalog(result))
...
def audit(
    ...
    log_path = path_option_value(log_file) or Path(policy.audit_log_path)
...
def report(
    ...
    if suffix == ".json":
        ...
        write_scan_outputs(result, bool(result.fix_plans), path_option_value(json_output), path_option_value(markdown_output), path_option_value(html_output))
        return
...
def baseline(
    ...
    output_path = path_option_value(output)
    audit_log_path = path_option_value(audit_log)
    policy = load_app_config(path_option_value(policy_file))
    timeout_seconds = timeout_option_value(timeout)
...
def compare(
    ...
    compare_outputs(comparison, markdown_output, html_output)
