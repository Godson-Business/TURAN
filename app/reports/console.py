...
def render_console(result: ScanResult, include_fix_plans: bool = False) -> Group:
    ...
    findings.add_column("Impact", style="white")
...
def render_interactive_fix_catalog(result: ScanResult) -> Table:
    catalog = Table(title="Suggested Fixes")
    ...
    if result.fix_plans:
        for index, (finding, plan) in enumerate(zip(result.findings, result.fix_plans), start=1):
...
def render_comparison(comparison: ComparisonResult) -> Group:
    ...
    return Group(summary, fixed, new)
